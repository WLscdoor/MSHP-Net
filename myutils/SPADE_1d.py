...
class SPADE(nn.Module):
    def __init__(self, feature_size, style_size):
        ...
        self.norm = nn.BatchNorm1d(feature_size, affine=False)
        self.conv = nn.Sequential(
            spectral_norm(nn.Conv1d(style_size, 128, 3, 1, 1)),
            nn.ReLU(inplace=True)
        )
        self.conv_gamma = spectral_norm(nn.Conv1d(128, feature_size, 3, 1, 1))
        self.conv_beta = spectral_norm(nn.Conv1d(128, feature_size, 3, 1, 1))
...
class SPADEResBlk_1d(nn.Module):
    def __init__(self, input_size, output_size, style_size):
        super(SPADEResBlk_1d, self).__init__()
        # Main layer
        self.spade_1 = SPADE(input_size, style_size)
        self.relu_1 = nn.ReLU(inplace=True)
        self.conv_1 = spectral_norm(nn.Conv1d(input_size, output_size, 3, 1, 1))
        self.spade_2 = SPADE(output_size, style_size)
        self.relu_2 = nn.ReLU(inplace=True)
        self.conv_2 = spectral_norm(nn.Conv1d(output_size, output_size, 3, 1, 1))
        # Shortcut layer
        self.spade_s = SPADE(input_size, style_size)
        self.relu_s = nn.ReLU(inplace=True)
        self.conv_s = spectral_norm(nn.Conv1d(input_size, output_size, 3, 1, 1))
...
# input_tensor = torch.rand(2, 16, 256)
# mask_tensor = torch.rand(2, 1, 256)
